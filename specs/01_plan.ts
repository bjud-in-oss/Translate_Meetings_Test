...
**Robust lösning (Init Script):**
För att garantera att \`netlify.toml\` och \`_redirects\` alltid finns vid byggtillfället (även om filerna rensas i roten), skapade vi ett skript \`scripts/init-netlify.js\` och uppdaterade \`package.json\` så att detta körs automatiskt vid \`npm run build\`.
`;